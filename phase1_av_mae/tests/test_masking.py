...
def test_output_shapes():
    B, N, D = 2, 196, 384
    x = torch.randn(B, N, D)
    x_vis, mask, ids_restore = random_masking(x, masking_ratio=0.4)
...
def test_mask_ratio():
    B, N, D = 4, 200, 128
    x = torch.randn(B, N, D)
    _, mask, _ = random_masking(x, masking_ratio=0.4)
...
def test_mask_binary():
    ...
    _, mask, _ = random_masking(x, masking_ratio=0.5)
    unique = mask.unique()
...
def test_ids_restore_is_permutation():
    B, N, D = 2, 50, 32
    x = torch.randn(B, N, D)
    _, _, ids_restore = random_masking(x, masking_ratio=0.4)
